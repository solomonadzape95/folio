  ...
  return (
    <div className="space-y-8">
        ...
        {results.length ? (
          <div className="border-t border-line p-5 sm:p-7">
            ...
            <div className="grid gap-px bg-line sm:grid-cols-2 lg:grid-cols-3">
              {results.slice(0, 9).map((book) => <SearchResult key={book.googleId} book={book} saved={savedIds.has(book.googleId)} onSave={() => save(book)} />)}
            </div>
          </div>
        ) : null}
      </section>
      ...
      <section>
        ...
        {visible.length ? <div className="grid gap-px border border-line bg-line md:grid-cols-2 xl:grid-cols-3">{visible.map((book) => <LibraryCard key={book.id} book={book} onStatus={changeStatus} onRemove={remove} />)}</div> : <div className="stripe border border-line px-6 py-24 text-center"><p className="font-mono text-[11px] uppercase tracking-[0.24em] text-faint">This shelf is waiting for its first book</p></div>}
      </section>
    </div>
  );
}
